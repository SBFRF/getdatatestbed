...
def getncfnameModel(prefix, local=True, model='STWAVE', server = None):
    """Function creates the url for the netCDF file interested.
    
    Doesn't yet get specific monthly file if applicable to speed things up.

    Args:
        dataLoc (str): 
        server (str): a key associated with the server location
        ...
        model (str): one of: STWAVE, CMS, celeris, swash (other models can be added)
    ...
    if prefix == 'CBHPStatic' and local == True:  # this is needed because projects are
        ...
        fname = loc + u'cmtb/waveModels/%s/%s/%s-Field/%s-Field.ncml' % (
            model, prefix, grid, grid)
    elif model == 'CMS':  # this is standard operational model url Structure
        fname = loc + u'cmtb/waveModels/%s/%s/Field/Field.ncml' % (model, prefix)
    elif model == 'swash':
        frame = loc + u'cmtb/waveModels/%s/%s/Field/Field.ncml' % (model, prefix)
    elif model == 'celeris':
        frame = loc + u'cmtb/waveModels/%s/%s/Field/Field.ncml' % (model, prefix)
...
class getObs:
    ...
    def getALT(self, gaugeName=None, removeMasked=True):
        ...
        if gaugeName not in gauge_list:
            ...
            self.dataloc = 'geomorphology/altimeter/Alt940-150-altimeter/Alt940-150-altimeter.ncml'
        elif gaugeName in ['Alt940-200']:
            self.dataloc = 'geomorphology/altimeter/Alt940-200-altimeter/Alt940-200-altimeter.ncml'
        elif gaugeName in ['Alt940-250']:
            self.dataloc = 'geomorphology/altimeter/Alt940-250-altimeter/Alt940-250-altimeter.ncml'
            ...
            self.dataloc = 'geomorphology/altimeter/Alt861-250-altimeter/Alt861-250-altimeter.ncml'
...
class getDataTestBed:
    ...
    def __init__(self, d1, d2,**kwargs):
        ...
        self.chlDataLoc = u'https://chlthredds.erdc.dren.mil/thredds/dodsC/cmtb/'
    ...
    def getModelField(self, var, prefix, local=True, model='STWAVE', server=None, **kwargs):
        ...
        # Check if var is list or single string
        if not isinstance(var,list):
            var = [var]

        # Determine if local or regional
        if local == False:
            grid = 'Regional'
        else:
            grid = 'Local'
        ...
        fname = getncfnameModel(prefix, grid, model, server)

        # Try opening multiple times, with delay between attempts
        finished = False
        n = 0
        ...
        # Determine if variable called is in file
        '''
        for v in var:
            assert v in ncfile.variables.keys(), 'variable called is not in file please use\n%s' % \
                                                ncfile.variables.keys()
        '''
        # Grab variables of interest from file
        ...
        # If xbounds and ybounds are specified grab data within bounds
        if 'xbounds' in kwargs and np.array(kwargs['xbounds']).size == 2:
            if kwargs['xbounds'][0] > kwargs['xbounds'][1]:
                kwargs['xbounds'] = np.flip(kwargs['xbounds'], axis=0)
            # first min of x
            ...
            if (kwargs['xbounds'][1] > ncfile['xFRF'][:]).all():
                ...
                removeMaxX = np.argwhere(
                    ncfile['xFRF'][:] >= kwargs['xbounds'][1]).squeeze().min() + 1  # python indexing
        ...
        for v in var:
            if ncfile[v].ndim > 2 and idx.shape[0] > 100:  # looping through ... if necessary
                vlist = np.round(
                    np.linspace(idx.min(), idx.max(), idx.max() - idx.min(), endpoint=True, dtype=int))
                # if idx.max() not in vlist:
                #     vlist = np.append(vlist, idx.max())
                xFRF = ncfile['xFRF'][x]
                yFRF = ncfile['yFRF'][y]
                if len(vlist) < 100:
                    dataVar = np.array(ncfile[v][np.squeeze(vlist)])
                    timeVar = nc.num2date(ncfile['time'][np.squeeze(vlist)], ncfile['time'].units,
                                        only_use_cftime_datetimes=False)
                else:
                    for num, minidx in enumerate(vlist):
                        if len(vlist) < 100:
                            dataVar = np.array(ncfile[v][np.squeeze(vlist)])
                            timeVar = nc.num2date(ncfile['time'][np.squeeze(vlist)],
                                                ncfile['time'].units,
                                                only_use_cftime_datetimes=False)
                        elif num == 0:
                            dataVar = np.array(ncfile[v][range(minidx, vlist[num + 1]), y, x])
                            timeVar = nc.num2date(
                                np.array(ncfile['time'][range(minidx, vlist[num + 1])]),
                                ncfile['time'].units,
                                only_use_cftime_datetimes=False)
                        elif minidx == vlist[-1]:
                            lastIdx = (idx - minidx)[(idx - minidx) >= 0] + minidx
                            dataVar = np.append(dataVar, ncfile[v][lastIdx, y, x], axis=0)
                            timeVar = np.append(timeVar, nc.num2date(ncfile['time'][lastIdx],
                                                                    ncfile['time'].units,
                                                                    only_use_cftime_datetimes=False), axis=0)
                        else:
                            dataVar = np.append(dataVar,
                                                ncfile[v][range(minidx, vlist[num + 1]), y, x], axis=0)
                            timeVar = np.append(timeVar, nc.num2date(
                                ncfile['time'][range(minidx, vlist[num + 1])],
                                ncfile['time'].units,
                                only_use_cftime_datetimes=False), axis=0)
            
            elif ncfile[v].ndim > 2:
                dataVar = ncfile[v][idx, y, x]
                xFRF = ncfile['xFRF'][x]
                yFRF = ncfile['yFRF'][y]
                timeVar = nc.num2date(ncfile['time'][np.squeeze(idx)], ncfile['time'].units,
                                    only_use_cftime_datetimes=False)
            else:  # probably bathymetry date variable
                dataVar = ncfile[v][idx]
                xFRF = ncfile['xFRF'][x]
                yFRF = ncfile['yFRF'][y]
                timeVar = nc.num2date(ncfile['time'][np.squeeze(idx)], ncfile['time'].units,
                                    only_use_cftime_datetimes=False)
        ...
        for v in var: 
            field[v]= dataVar
            assert field[v].shape[0] == len(field['time']), " the indexing is wrong for pulling down the spatial output"
    ...
    def getWaveSpecModel(self, prefix, gaugenumber, model='STWAVE', removeBadWLFlag=True):
        ...
        self.ncfile, self.allEpoch = getnc(dataLoc=self.dataloc, callingClass=self.callingClass,
                                           dtRound=1 * 60, start=self.d1, end=self.d2, server=self.server)
